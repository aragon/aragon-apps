...
start_ganache() {
  ...
  npx ganache-cli -i ${NETWORK_ID} -l ${GAS_LIMIT} -e ${BALANCE} -p ${PORT} --keepAliveTimeout 15000 > /dev/null &
  rpc_pid=$!
  sleep 3
...
start_testrpc() {
  ...
  npx testrpc-sc -i ${NETWORK_ID} -l ${GAS_LIMIT} -e ${BALANCE} -p ${PORT} --keepAliveTimeout 15000 > /dev/null &
  rpc_pid=$!
  sleep 3

...
const bigExp = (x, y) => web3.utils.toBN(x).mul(web3.utils.toBN(10).pow(web3.utils.toBN(y)))
...
contract('Voting App', ([root, holder1, holder2, holder20, holder29, holder51, nonHolder]) => {
        ...
        context(`normal token supply, ${decimals} decimals`, () => {
            ...
            context('creating vote', () => {
                ...
                it('has correct state', async () => {
                    const data = await voting.getVote(voteId)

                    assert.isTrue(data.open, 'vote should be open')
                    assert.isFalse(data.executed, 'vote should not be executed')
                    assert.equal(creator, holder51, 'creator should be correct')
                    assert.equal(data.snapshotBlock.toString(), await getBlockNumber() - 1, 'snapshot block should be correct')
                    assert.equal(data.supportRequired.toString(), neededSupport.toString(), 'required support should be app required support')
                    assert.equal(data.minAcceptQuorum.toString(), minimumAcceptanceQuorum.toString(), 'min quorum should be app min quorum')
                    assert.equal(data.yea, 0, 'initial yea should be 0')
                    assert.equal(data.nay, 0, 'initial nay should be 0')
                    assert.equal(data.votingPower.toString(), bigExp(100, decimals).toString(), 'voting power should be 100')
                    assert.equal(data.script, script, 'script should be correct')
    ...
    context('token supply = 1', () => {
        ...
        it('new vote cannot be executed before voting', async () => {
            ...
            assert.isFalse(data.open, 'vote should be closed')
            assert.isTrue(data.executed, 'vote should have been executed')
        ...
        context('new vote parameters', () => {
            ...
            it("creating vote as holder doesn't execute vote if _canExecute param doesn't says so", async () => {
                ...
                assert.isTrue(data.open, 'vote should be open')
                assert.isFalse(data.executed, 'vote should not have been executed')
            })
        })
    })
    ...
    context('changing token supply', () => {
        ...
        it('uses the correct snapshot value if tokens are minted afterwards', async () => {
            ...
            assert.equal(data.snapshotBlock.toString(), await getBlockNumber() - 2, 'snapshot block should be correct')
            assert.equal(data.votingPower.toString(), (await token.totalSupplyAt(data.snapshotBlock)).toString(), 'voting power should match snapshot supply')
            assert.equal(data.votingPower.toString(), 2, 'voting power should be correct')
        })

...
contract("Finance App", ([root, owner, recipient]) => {
  ...
  it("sets the end of time correctly", async () => {
    ...
    const [isCurrent, start, end, firstTx, lastTx] = await financeApp.getPeriod(
  ...
  it("fails on initializing with no vault", async () => {
    ...
    await assertRevert(
      financeApp.initialize(0, PERIOD_DURATION),
    ...
    context(`ERC20 (${title}) deposits`, () => {
      ...
      it("records deposits", async () => {
        ...
        const [
          ...
          amount,
          paymentId,
          paymentExecutionNumber,
          token,
          entity,
          incoming,
          date
        ] = await finance.getTransaction(1);
  ...
  context("ETH deposits", () => {
    ...
    it("records deposits using deposit function", async () => {
      ...
      const [
        ...
        incoming,
        date
      ] = await finance.getTransaction(transactionId);
  ...
  context("setting budget", () => {
    ...
    it("can create scheduled payment", async () => {
      ...
      return Promise.all(
        [firstReceipt, secondReceipt].map(async (receipt, index) => {
          ...
          assert.equal(
            paymentExecutionNumber.valueOf(),
            executionNum,
    ...
    context("multitransaction period", async () => {
      ...
      it("has correct token statements", async () => {
        const [t1expense, t1income] = await finance.getPeriodTokenStatement(
          0,
          token1.address
        );
        const [t2expense, t2income] = await finance.getPeriodTokenStatement(
          0,
          token2.address
        );

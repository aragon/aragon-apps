...
contract("Finance App", ([root, owner, recipient]) => {
  ...
  const newProxyFinance = async () => {
    ...
    const dao = await getContract("Kernel").at(
      getEventArgument(r, "DeployDAO", "dao")
    );
    ...
    // finance
    const receipt2 = await dao.newAppInstance(
      "0x5678",
      financeBase.address,
      "0x",
      false,
  ...
  beforeEach(async () => {
    ...
    await vault.deposit(ETH, VAULT_INITIAL_ETH_BALANCE, {
      value: VAULT_INITIAL_ETH_BALANCE,
      from: owner
    });
  ...
  it("initialized first accounting period and settings", async () => {
    ...
    assert.equal(
      await finance.currentPeriodId(),
      0,
      "current period should be 0"
    );
  });
  ...
  it("sets the end of time correctly", async () => {
    ...
    const data = await financeApp.getPeriod(
      await financeApp.currentPeriodId()
    );

    assert.deepEqual(
      data.endTime,
      MAX_UINT64,
      "should have set the period's end date to MAX_UINT64"
    );
  });

  it("fails on reinitialization", async () => {
    await assertRevert(
      finance.initialize(vault.address, PERIOD_DURATION),
  ...
  it("cannot initialize base app", async () => {
    ...
    await assertRevert(
      newFinance.initialize(vault.address, PERIOD_DURATION),
      errors.INIT_ALREADY_INITIALIZED
    );
  });
  ...
  it("fails on initializing with no vault", async () => {
    ...
    await assertRevert(
      financeApp.initialize('0x0000000000000000000000000000000000000000', PERIOD_DURATION),
      errors.FINANCE_VAULT_NOT_CONTRACT
    );
    await assertRevert(
      financeApp.initialize(withdrawAddr, PERIOD_DURATION),
      errors.FINANCE_VAULT_NOT_CONTRACT
    );
  });
  ...
  it("fails on initializing with less than one day period", async () => {
    ...
    await assertRevert(
      financeApp.initialize(vault.address, badPeriod),
      errors.FINANCE_SET_PERIOD_TOO_SHORT
    );
  });
  ...
  it("adds new token to budget", async () => {
    ...
    const data = await finance.getBudget.call(token1.address);

    const remainingBudget = await finance.getRemainingBudget.call(
      token1.address
    );
    assert.equal(data.budget, 10, "should have correct budget");
    assert.isTrue(data.hasBudget, "has budget should be true");
  ...
  it("before setting budget allows unlimited spending", async () => {
    ...
    assert.equal(
      (await token2.balanceOf(recipient)).valueOf(),
      amount,
  ...
  it("can change period duration", async () => {
    ...
    assert.equal(
      (await finance.currentPeriodId()).valueOf(),
      2,
      "should have transitioned 2 periods"
    );
  });
  ...
  it("only transitions as many periods as allowed", async () => {
    ...
    assert.equal(
      await finance.currentPeriodId(),
      1,
      "should have transitioned 1 periods"
    );
  });
  ...
  it("fails on changing period duration to too short", async () => {
    const badPeriod = 60 * 60 * 24 - 1;
    await assertRevert(
      finance.setPeriodDuration(badPeriod),
      errors.FINANCE_SET_PERIOD_TOO_SHORT
    );
  });
    ...
    context(`ERC20 (${title}) deposits`, () => {
      ...
      beforeEach(async () => {
        ...
        tokenInstance = await tokenContract.new(
          owner,
          10000 + VAULT_INITIAL_TOKEN1_BALANCE
        );
        await tokenInstance.transfer(
          vault.address,
          VAULT_INITIAL_TOKEN1_BALANCE,
          { from: owner }
        );
      });
      ...
      it("records deposits", async () => {
        ...
        const receipt = await finance.deposit(
        ...
        assert.equal(
          (await tokenInstance.balanceOf(vault.address)).valueOf(),
          VAULT_INITIAL_TOKEN1_BALANCE + transferAmount,
          "deposited tokens must be in vault"
        );

        const data = await finance.getTransaction(1);

        assert.equal(data.periodId, 0, "period id should be correct");
        assert.equal(data.amount, transferAmount, "amount should be correct");
        assert.equal(data.paymentId, 0, "payment id should be 0");
        assert.equal(
          data.paymentExecutionNumber,
          0,
          "payment execution number should be 0"
        );
        assert.equal(data.token, tokenInstance.address, "token should be correct");
        assert.equal(data.entity, owner, "entity should be correct");
        assert.isTrue(data.isIncoming, "tx should be incoming");
        assert.equal(data.date, 1, "date should be correct");
      ...
      it("fails on no value deposits", async () => {
        await assertRevert(
          finance.deposit(tokenInstance.address, 0, "ref"),
          errors.FINANCE_DEPOSIT_AMOUNT_ZERO
        );
      });
    });
  }
  ...
  context("ETH deposits", () => {
    ...
    it("records deposits using deposit function", async () => {
      const receipt = await finance.deposit(ETH, sentWei, reference, {
        from: owner,
        value: sentWei
      });

      const transactionId = getEventArgument(
        receipt,
        "NewTransaction",
        "transactionId"
      );

      const data = await finance.getTransaction(transactionId);

      assert.equal(
        await vault.balance(ETH),
        VAULT_INITIAL_ETH_BALANCE + sentWei,
        "deposited ETH must be in vault"
      );
      assert.equal(data.periodId, 0, "period id should be correct");
      assert.equal(data.amount, sentWei, "amount should be correct");
      assert.equal(data.paymentId, 0, "payment id should be 0");
      assert.equal(
        data.paymentExecutionNumber,
        0,
      ...
      assert.equal(data.token, ETH, "token should be ETH token");
      assert.equal(data.entity, owner, "entity should be correct");
      assert.isTrue(data.isIncoming, "tx should be incoming");
      assert.equal(data.date, 1, "date should be correct");
    ...
    it("records ETH deposits using fallback", async () => {
      const receipt = await finance.sendTransaction({
        from: owner,
        value: sentWei
      });
      const transactionId = getEventArgument(
    ...
    it("fails to deposit if amount does not match value", async () => {
      await assertRevert(
        finance.deposit(ETH, sentWei - 1, reference, { value: sentWei }),
        errors.FINANCE_ETH_VALUE_MISMATCH
      );
    });
  });
  ...
  for (const { title, tokenContract } of tokenTestGroups) {
    context(`locked ERC20 (${title})`, () => {
      ...
      beforeEach(async () => {
        ...
        tokenInstance = await tokenContract.new(
          owner,
          10000 + VAULT_INITIAL_TOKEN1_BALANCE + lockedTokenAmount
        );
        await tokenInstance.transfer(
          vault.address,
          VAULT_INITIAL_TOKEN1_BALANCE,
      ...
      it("allow recoverability is disabled", async () => {
        assert.isFalse(
          await finance.allowRecoverability(tokenInstance.address)
        );
      });
      ...
      it("are recovered using Finance#recoverToVault", async () => {
        ...
        assert.equal(
          await tokenInstance.balanceOf(finance.address),
          0,
          "finance shouldn't have tokens"
        );

        const data = await finance.getTransaction(1);
        assert.equal(data.periodId, 0, "period id should be correct");
        assert.equal(data.amount, lockedTokenAmount, "amount should be correct");
        assert.equal(data.paymentId, 0, "payment id should be 0");
        assert.equal(
          data.paymentExecutionNumber,
          0,
        ...
        assert.equal(data.token, tokenInstance.address, "token should be correct");
        assert.equal(data.entity, finance.address, "entity should be correct");
        assert.isTrue(data.isIncoming, "tx should be incoming");
        assert.equal(data.date, 1, "date should be correct");

        assertEvent(receipt, "NewTransaction", {
          reference: "Recover to Vault"
        });
      });
      ...
      it("fail to be recovered if token balance is 0", async () => {
        ...
        await assertRevert(
          ...
          errors.FINANCE_RECOVER_AMOUNT_ZERO
        );
      });
    });
  }
  ...
  context("locked ETH", () => {
    ...
    beforeEach(async () => {
      ...
      assert.equal(
        (await getBalance(finance.address)).valueOf(),
        lockedETH,
        "finance should have stuck ETH"
      );
    });
    ...
    it("is recovered using Finance#recoverToVault", async () => {
      ...
      assert.equal(
        await vault.balance(ETH),
        VAULT_INITIAL_ETH_BALANCE + lockedETH,
        "recovered ETH must be in vault"
      );
      assert.equal(
        (await getBalance(finance.address)).valueOf(),
        0,
        "finance shouldn't have ETH"
      );
      assert.equal(data.periodId, 0, "period id should be correct");
      assert.equal(data.amount, lockedETH, "amount should be correct");
      assert.equal(data.paymentId, 0, "payment id should be 0");
      assert.equal(
        data.paymentExecutionNumber,
        0,
      ...
      assert.equal(data.token, ETH, "token should be correct");
      assert.equal(data.entity, finance.address, "entity should be correct");
      assert.isTrue(data.isIncoming, "tx should be incoming");
      assert.equal(data.date, 1, "date should be correct");
  ...
  context("setting budget", () => {
    ...
    it("records payment", async () => {
      ...
      const receipt = await finance.newScheduledPayment(
        token1.address,
        recipient,
        amount,
        NOW,
      ...
      const data = await finance.getPayment(1);

      assert.equal(data.token, token1.address, "token address should match");
      assert.equal(data.receiver, recipient, "receiver should match");
      assert.equal(data.amount, amount, "amount should match");
      assert.equal(data.initialPaymentTime, NOW, "time should match");
      assert.equal(data.interval, 2, "interval should match");
      assert.equal(data.maxExecutions, 10, "max executionss should match");
      assert.isFalse(data.inactive, "should be enabled");
      assert.equal(data.executions, 1, "should be on first execution");
      assert.equal(data.createdBy, root, "should have correct creator");
    });

    it("fails trying to get payment out of bounds", async () => {
      const amount = 10;
      // executes up to 10 times every 2 seconds
      await finance.newScheduledPayment(
        token1.address,
        recipient,
        amount,
        NOW,
      ...
      await assertRevert(
        finance.getPayment(0),
        errors.FINANCE_NO_SCHEDULED_PAYMENT
      );
      await assertRevert(
        finance.getPayment(2),
        errors.FINANCE_NO_SCHEDULED_PAYMENT
      );
    });
    ...
    it("fails trying to get transaction out of bounds", async () => {
      ...
      await assertRevert(
        finance.getTransaction(2),
        errors.FINANCE_NO_TRANSACTION
      );
    });
    ...
    it("can create single payment transaction", async () => {
      ...
      const receipt = await finance.newImmediatePayment(
        token1.address,
        recipient,
        amount,
        "ref"
      ...
      assert.equal(
        (await token1.balanceOf(recipient)).valueOf(),
        amount,
        "recipient should have received tokens"
      );
      ...
      assert.equal(data.periodId, 0, "period id should be correct");
      assert.equal(data.amount, amount, "amount should match");
      assert.equal(data.paymentId, 0, "payment id should be 0 for single payment");
      assert.equal(
        data.paymentExecutionNumber,
        0,
        "payment execution number should be 0"
      ...
      assert.equal(data.token, token1.address, "token address should match");
      assert.equal(data.entity, recipient, "receiver should match");
      assert.isFalse(data.isIncoming, "single payment should be outgoing");
      assert.equal(data.date.valueOf(), NOW, "date should be correct");
    });
    ...
    it("can decrease budget after spending", async () => {
      ...
      assert.equal(data.budget, newBudgetAmount, "new budget should be correct");
      assert.isTrue(data.hasBudget, "should have budget");
    ...
    it("removing budget allows unlimited spending", async () => {
      ...
      const data = await finance.getBudget.call(token2.address);
      assert.equal(data.budget, 0, "removed budget should be 0");
      assert.isFalse(data.hasBudget, "should not have budget");
    ...
    it("can create scheduled payment", async () => {
      ...
      // executes up to 10 times every 2 seconds
      const firstReceipt = await finance.newScheduledPayment(
        token1.address,
        recipient,
        amount,
        NOW,
      ...
      assert.equal(
        await finance.nextPaymentTime(1),
        NOW + 4 + 2,
        "payment should be executed again in 2"
      );

      return Promise.all(
        [firstReceipt, secondReceipt].map(async (receipt, index) => {
          ...
          assert.equal(data.amount, amount, "amount should be correct");
          assert.equal(data.paymentId, 1, "payment id should be 1");
          assert.equal(
            data.paymentExecutionNumber.valueOf(),
            executionNum,
            `payment execution number should be ${executionNum}`
          );
        })
      );
    });
    ...
    it("can create scheduled ether payment", async () => {
      ...
      assert.equal(
        (await getBalance(withdrawAddr)).valueOf(),
        amount * 3,
        "recipient should have received ether"
      );
    });
    ...
    it("doesnt record payment for single payment transaction", async () => {
      ...
      assertAmountOfEvents(receipt, "NewPayment", 0);
      await assertRevert(
        finance.getPayment(1),
        errors.FINANCE_NO_SCHEDULED_PAYMENT
      );
    });
    ...
    context("multitransaction period", async () => {
      ...
      it("has correct token statements", async () => {
        const data1 = await finance.getPeriodTokenStatement(
          0,
          token1.address
        );
        const data2 = await finance.getPeriodTokenStatement(
          0,
          token2.address
        );

        assert.equal(data1.expenses, 10, "token 1 expenses should be correct");
        assert.equal(data1.income, 5, "token 1 income should be correct");

        assert.equal(data2.expenses, 10, "token 2 expenses should be correct");
        assert.equal(data2.income, 0, "token 2 income should be correct");
      });
      ...
      it("finishes accounting period correctly", async () => {
        ...
        const data = await finance.getPeriod(0);

        assert.isFalse(data.isCurrent, "shouldnt be current period");
        assert.equal(data.startTime.toNumber(), NOW, "should have correct start date");
        assert.equal(
          data.endTime.toNumber(),
          NOW + PERIOD_DURATION - 1,
          "should have correct end date"
        );
        assert.equal(data.firstTransactionId.valueOf(), 1, "should have correct first tx");
        assert.equal(data.lastTransactionId.valueOf(), 4, "should have correct last tx");
      });
      ...
      it("fails trying to access period out of bounds", async () => {
        ...
        await assertRevert(
          finance.getPeriod(currentPeriodId + 1),
          errors.FINANCE_NO_PERIOD
        );
      });
    });
    ...
    context("many accounting period transitions", () => {
      ...
      it("non-activity accounting periods have no transactions", async () => {
        ...
        const data = await finance.getPeriod(2);

        assert.isFalse(data.isCurrent, "shouldnt be current period");
        assert.equal(
          data.startTime.toNumber(),
          PERIOD_DURATION * 2 + 1,
        ...
        assert.equal(data.firstTransactionId, 0, "should have empty txs");
        assert.equal(data.lastTransactionId, 0, "should have empty txs");
      });
    });
    ...
    context("single payment", async () => {
      ...
      it("can create a single payment", async () => {
        ...
        assertAmountOfEvents(receipt, "NewTransaction", 1);
        assertAmountOfEvents(receipt, "NewPeriod", 0);
      });
    ...
    context("scheduled payment", async () => {
      ...
      it("can create a scheduled payment", async () => {
        ...
        assertAmountOfEvents(receipt, "NewPayment", 1);
      });
      ...
      it("fails to execute a payment without enough funds", async () => {
        ...
        const newScheduledPaymentId = getEventArgument(
          receipt,
          "NewPayment",
          "paymentId"
        );
        ...
        await assertRevert(
          finance.executePayment(newScheduledPaymentId),
      ...
      it("fails to execute a payment by receiver without enough funds", async () => {
        ...
        await assertRevert(
          finance.receiverExecutePayment(newScheduledPaymentId, {
            from: recipient
          }),
          errors.FINANCE_EXECUTE_PAYMENT_NUM
        );
      ...
      context("executing scheduled payment", async () => {
        ...
        it("only executes payment until max executions", async () => {
          ...
          assert.deepEqual(
            await finance.nextPaymentTime(paymentId),
            MAX_UINT64,
            "payment should never be repeated"
          );
        });
        ...
        it("fails when non-receiver attempts to execute a payment", async () => {
          ...
          await assertRevert(
            finance.receiverExecutePayment(paymentId),
            errors.FINANCE_PAYMENT_RECEIVER
          );
        });

        it("fails when executing before next available time", async () => {
          await assertRevert(
            finance.executePayment(paymentId),
      ...
      context("payment failure", async () => {
        it("tries to execute a new scheduled payment if initially possible even without enough funds", async () => {
          ...
          assertAmountOfEvents(receipt, "PaymentFailure", 1);
          // Make sure no transactions were made
          assertAmountOfEvents(receipt, "NewTransaction", 0);
        });
        ...
        it("emits payment failure event when out of balance", async () => {
          ...
          const paidTimes = Math.floor(
            (await vault.balance(token1.address)) / amountPerPayment
          );
          ...
          assertAmountOfEvents(receipt, "PaymentFailure", 1);
          assert.equal(
            await token1.balanceOf(recipient),
            amountPerPayment * paidTimes,
            "recipient should have received tokens"
          );
          assert.isFalse(
            await finance.canMakePayment(token1.address, amountPerPayment)
  ...
  context("Without initialize", async () => {
    ...
    it("fails to deposit ERC20 tokens", async () => {
      ...
      await assertRevert(
        nonInit.deposit(token1.address, 5, "ref"),
    ...
    context("locked ERC20", () => {
      ...
      it("fails to be recovered using Finance#recoverToVault", async () => {
        await assertRevert(
          nonInit.recoverToVault(token1.address),
          errors.INIT_NOT_INITIALIZED
        );
      });
    });
    ...
    context("locked ETH", () => {
      ...
      beforeEach(async () => {
        ...
        assert.equal(
          (await getBalance(nonInit.address)).valueOf(),
          lockedETH,
          "finance should have stuck ETH"
        );
      });

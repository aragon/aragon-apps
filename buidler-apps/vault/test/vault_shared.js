    ...
    context('> Initialized', () => {
      ...
      context('> ETH', () => {
        ...
        it('transfers ETH to contract', async () => {
          ...
          await vault.sendTransaction( { value: depositValue })
          const initialBalance = web3.utils.toBN(await getBalance(destination.address))
      ...
      for (const { title, tokenContract} of tokenTestGroups) {
        context(`> ERC20 (${title})`, () => {
          ...
          it('deposits ERC20s', async () => {
            ...
            assert.deepEqual(await vault.balance(token.address), vaultBalance, "vault should know its balance")
          })
      ...
      context('> Recovering assets', () => {
        ...
        beforeEach(async () => {
          ...
          defaultVault = await VaultLike.at(getNewProxyAddress(defaultVaultReceipt))

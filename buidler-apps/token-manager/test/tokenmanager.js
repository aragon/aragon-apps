    ...
    for (const tokenTransferable of [true, false]) {
        context(`for ${tokenTransferable ? 'transferable' : 'non-transferable'} tokens`, () => {
            ...
            it("cannot call proxyPayment() from outside of the token's context", async () => {
                ...
                assert.equal(await getBalance(tokenManager.address), prevTokenManagerBalance, 'token manager ETH balance should be the same')
            })
            ...
            if (tokenTransferable) {
                context('assigning vested tokens', () => {
                    ...
                    it('can get vesting details before being revoked', async () => {
                        const vestingData = await tokenManager.getVesting(holder, 0)

                        assert.equal(vestingData.amount, totalTokens)
                        assert.equal(vestingData.start, startDate)
                        assert.equal(vestingData.cliff, cliffDate)
                        assert.equal(vestingData.vesting, vestingDate)
                        assert.equal(vestingData.revokable, revokable)
                    })

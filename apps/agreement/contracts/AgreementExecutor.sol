...
contract AgreementExecutor is IArbitrable, IForwarder, AragonApp {
    ...
    /* Arbitrable related errors */
    string internal constant ERROR_ARBITRABLE_NOT_CONTRACT = "AGR_ARBITRABLE_NOT_CONTRACT";
    ...
    // bytes32 public constant CHANGE_AGREEMENT_SETTING_ROLE = keccak256("CHANGE_AGREEMENT_SETTING_ROLE");
    bytes32 public constant CHANGE_AGREEMENT_SETTING_ROLE = 0x4cfaa48ab124a7ae99a57caff22dd585bc5e2bd0e98c8dd36a36f1d4c856b44b;
    ...
    struct Action {
        ...
        uint256 settingId;              // Identification number of the Agreement Executor setting when the action was scheduled
        uint256 contentId;              // Identification number of the Agreement content when the action was scheduled
    ...
    */
    ...
    Agreement public agreement;         // Agreement app holding the core details
    ...
    /**
    * @notice Initialize Agreement Executor app for Agreement `_agreement` with:
    ...
    * @param _agreement Agreement app holding the core details
    ...
    */
    ...
    {
        initialized();
        require(isContract(address(_agreement)), ERROR_ARBITRABLE_NOT_CONTRACT);
        ...
        _newSetting(_delayPeriod, _settlementPeriod, _collateralAmount, _challengeCollateral);
    }
    ...
    */
    ...
    /**
    * @notice Change Agreement executor configuration parameters to
    ...
    * @param _challengeCollateral Amount of `collateralToken` that will be locked every time an action is challenged
    */
    function changeSetting(uint64 _delayPeriod, uint64 _settlementPeriod, uint256 _collateralAmount, uint256 _challengeCollateral)
        external
        auth(CHANGE_AGREEMENT_SETTING_ROLE)
    {
        _newSetting(_delayPeriod, _settlementPeriod, _collateralAmount, _challengeCollateral);
    }
    ...
    */
    ...
    /**
    * @dev Tell the information related to a signer address
    ...
    */
    function getSigner(address _signer) external view returns (uint256 available, uint256 locked, uint256 challenged) {
    ...
    * @return settingId Identification number of the Agreement Executor setting when the action was scheduled
    * @return contentId Identification number of the Agreement content when the action was scheduled
    */
    ...
    */
        ...
        )
    {
        ...
        collateralAmount = setting.collateralAmount;
        delayPeriod = setting.delayPeriod;
        settlementPeriod = setting.settlementPeriod;
        challengeCollateral = setting.challengeCollateral;
    }
    ...
    */
    function _createAction(address _submitter, bytes _context, bytes _script) internal {
        (, bool mustSign) = agreement.getSigner(_submitter);
        require(!mustSign, ERROR_SIGNER_MUST_SIGN_SETTING);
        ...
        action.contentId = agreement.getCurrentContentId();
    ...
    */
    function _createDispute(Action storage _action, Setting storage _setting) internal returns (uint256) {
        ...
        // Create dispute
        require(feeToken.safeTransferFrom(_action.submitter, address(this), missingFees), ERROR_ARBITRATOR_FEE_DEPOSIT_FAILED);
        ...
        uint256 disputeId = arbitrator.createDispute(DISPUTES_POSSIBLE_OUTCOMES, agreement.getContent(_action.contentId));

        // Update action and submit evidences
        _submitEvidence(disputeId, _action.submitter, _action.context, false);
        _submitEvidence(disputeId, challenge.challenger, challenge.context, false);

        // Return arbitrator fees to challenger if necessary
        if (challenge.arbitratorFeeToken != feeToken) {
            require(challengerFeeToken.safeTransfer(challenge.challenger, challengerFeeAmount), ERROR_ARBITRATOR_FEE_RETURN_FAILED);
        }
        return disputeId;
    }
    ...
    * @param _challengeCollateral Amount of `collateralToken` that will be locked every time an action is challenged
    */
    function _newSetting(uint64 _delayPeriod, uint64 _settlementPeriod, uint256 _collateralAmount, uint256 _challengeCollateral) internal {
    ...
    */
    function _canSign(address _signer) internal view returns (bool) {
        return agreement.canSign(_signer);
    }
    ...
    */
    function _canChallenge(address _challenger) internal view returns (bool) {
        return agreement.canChallenge(_challenger);
    }

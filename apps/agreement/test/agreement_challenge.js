...
contract('Agreement', ([_, submitter, challenger]) => {
  ...
  describe('challenge', () => {
    context('when the given action exists', () => {
      ...
      const itCannotBeChallenged = () => {
        it('reverts', async () => {
          await assertRevert(agreement.challenge({ actionId }), ERRORS.ERROR_CANNOT_CHALLENGE_ACTION)
        })
      }
          ...
          const itChallengesTheActionProperly = () => {
            context('when the challenger has staked enough collateral', () => {
              ...
              context('when the challenger approved less than half of the arbitration fees', () => {
                ...
                it('reverts', async () => {
                  await assertRevert(agreement.challenge({ actionId, arbitrationFees }), ERRORS.ERROR_ARBITRATOR_FEE_TRANSFER_FAILED)
                })
              })
            ...
            context('when the challenger did not stake enough collateral', () => {
              ...
              it('reverts', async () => {
                await assertRevert(agreement.challenge({ actionId, stake, arbitrationFees }), ERRORS.ERROR_COLLATERAL_TOKEN_TRANSFER_FAILED)
              })
            })
          }
  ...
  context('when the given action does not exist', () => {
    it('reverts', async () => {
      await assertRevert(agreement.challenge({ actionId: 0 }), ERRORS.ERROR_ACTION_DOES_NOT_EXIST)
    })
  })
})

...
contract('Voting disputable', ([_, owner, representative, voter10, voter20, voter29, voter40]) => {
  ...
  beforeEach('create voting app', async () => {
    voting = await deployer.deployAndInitialize({ owner, requiredSupport: MIN_SUPPORT, minimumAcceptanceQuorum: MIN_QUORUM, voteDuration: VOTING_DURATION, quietEndingPeriod: QUIET_ENDING_PERIOD, quietEndingExtension: QUIET_ENDING_EXTENSION, delegatedVotingPeriod: DELEGATED_VOTING_PERIOD, executionDelay: EXECUTION_DELAY })
  })
  ...
  describe('newVote', () => {
    ...
    it('can be paused during the execution delay window', async () => {
    ...
    it('cannot be paused if ready to be executed', async () => {
      await voting.vote(voteId, true, { from: voter40 })
      await voting.mockIncreaseTime(VOTING_DURATION + EXECUTION_DELAY)
  ...
  describe('challenge', () => {
    ...
    context('when the vote has not ended', () => {
    ...
    context('when the vote has ended', () => {
      beforeEach('end vote and challenge', async () => {
        await voting.vote(voteId, true, { from: voter40 })
        await voting.mockIncreaseTime(VOTING_DURATION)
        currentTimestamp = await voting.getTimestampPublic()

        const { isOpen } = await getVoteState(voting, voteId)
        assert.isFalse(isOpen, 'vote is not open')
  ...
  describe('resumes', () => {
    ...
    context('when the vote has not ended yet', () => {
      beforeEach('challenge vote', async () => {
    ...
    context('when the vote has ended', () => {
      ...
      const itDoesNotAllowCastingVotes = () => {
        it('does not allow voter to vote', async () => {
          assert.isFalse(await voting.canVote(voteId, voter29), 'voter cannot vote')
          await assertRevert(voting.vote(voteId, true, { from: voter29 }), VOTING_ERRORS.VOTING_CANNOT_VOTE)
        })

        it('does not allow to immediately execute votes', async () => {
          const { closed } = await agreement.getAction(actionId)
          assert.isFalse(closed, 'action is closed')

          assert.isFalse(await voting.canExecute(voteId), 'voting cannot be executed')
          await assertRevert(voting.executeVote(voteId, script), VOTING_ERRORS.VOTING_CANNOT_EXECUTE)
        })

        it('allows to execute after the execution delay', async () => {
          const beforeExecutionDelayEndDate = currentTimestamp.add(bn(EXECUTION_DELAY)).sub(bn(1))
          await voting.mockSetTimestamp(beforeExecutionDelayEndDate)

          const { isOpen: isOpenBeforeEndDate } = await getVoteState(voting, voteId)
          assert.isFalse(isOpenBeforeEndDate, 'vote is open before execution delay end date')

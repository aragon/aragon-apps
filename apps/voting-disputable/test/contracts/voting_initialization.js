...
contract('Voting initialization', ([_, owner]) => {
  ...
  describe('initialize', () => {
    it('cannot initialize base app', async () => {
      ...
      await assertRevert(votingBase.initialize(token.address, REQUIRED_SUPPORT, MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, DELEGATED_VOTING_PERIOD, QUIET_ENDING_PERIOD, QUIET_ENDING_EXTENSION, EXECUTION_DELAY), ARAGON_OS_ERRORS.INIT_ALREADY_INITIALIZED)
    })
    ...
    context('when the app was not initialized', () => {
      ...
      it('fails if acceptance quorum is greater than min support', async () => {
        ...
        await assertRevert(voting.initialize(token.address, requiredSupport, minimumAcceptanceQuorum, VOTE_DURATION, DELEGATED_VOTING_PERIOD, QUIET_ENDING_PERIOD, QUIET_ENDING_EXTENSION, EXECUTION_DELAY), VOTING_ERRORS.VOTING_CHANGE_QUORUM_TOO_BIG)
      })

      it('fails if support is 100% or more', async () => {
        await assertRevert(voting.initialize(token.address, pct16(101), MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, DELEGATED_VOTING_PERIOD, QUIET_ENDING_PERIOD, QUIET_ENDING_EXTENSION, EXECUTION_DELAY), VOTING_ERRORS.VOTING_CHANGE_SUPPORT_TOO_BIG)
        await assertRevert(voting.initialize(token.address, pct16(100), MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, DELEGATED_VOTING_PERIOD, QUIET_ENDING_PERIOD, QUIET_ENDING_EXTENSION, EXECUTION_DELAY), VOTING_ERRORS.VOTING_CHANGE_SUPPORT_TOO_BIG)
      })

      it('fails if the quiet ending period is greater than the vote duration', async () => {
        const quietEndingPeriod = VOTE_DURATION + 1

        await assertRevert(voting.initialize(token.address, REQUIRED_SUPPORT, MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, DELEGATED_VOTING_PERIOD, quietEndingPeriod, QUIET_ENDING_EXTENSION, EXECUTION_DELAY), VOTING_ERRORS.VOTING_INVALID_QUIET_END_PERIOD)
      })
    })

    context('when the app is already initialized', () => {
      before('initialize app', async () => {
        await voting.initialize(token.address, REQUIRED_SUPPORT, MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, DELEGATED_VOTING_PERIOD, QUIET_ENDING_PERIOD, QUIET_ENDING_EXTENSION, EXECUTION_DELAY)
      })

      it('cannot be re-initialized', async () => {
        await assertRevert(voting.initialize(token.address, REQUIRED_SUPPORT, MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, DELEGATED_VOTING_PERIOD, QUIET_ENDING_PERIOD, QUIET_ENDING_EXTENSION, EXECUTION_DELAY), ARAGON_OS_ERRORS.INIT_ALREADY_INITIALIZED)
      })
      ...
      it('is initialized correctly', async () => {
        ...
        const { supportRequiredPct, minAcceptQuorumPct, executionDelay, delegatedVotingPeriod, quietEndingPeriod, quietEndingExtension } = await voting.getSetting(currentSettingId)

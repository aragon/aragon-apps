...
contract('Voting initialization', ([_, owner]) => {
  ...
  describe('initialize', () => {
    it('cannot initialize base app', async () => {
      ...
      await assertRevert(votingBase.initialize(token.address, NEEDED_SUPPORT, MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, OVERRULE_WINDOW), ARAGON_OS_ERRORS.INIT_ALREADY_INITIALIZED)
    })
    ...
    context('when the app was not initialized', () => {
      ...
      it('fails if acceptance quorum is greater than min support', async () => {
        ...
        await assertRevert(voting.initialize(token.address, neededSupport, minimumAcceptanceQuorum, VOTE_DURATION, OVERRULE_WINDOW), VOTING_ERRORS.VOTING_CHANGE_QUORUM_PCTS)
      })

      it('fails if support is 100% or more', async () => {
        const minimumAcceptanceQuorum = pct(20)

        await assertRevert(voting.initialize(token.address, pct(101), minimumAcceptanceQuorum, VOTE_DURATION, OVERRULE_WINDOW), VOTING_ERRORS.VOTING_CHANGE_SUPP_TOO_BIG)
        await assertRevert(voting.initialize(token.address, pct(100), minimumAcceptanceQuorum, VOTE_DURATION, OVERRULE_WINDOW), VOTING_ERRORS.VOTING_CHANGE_SUPP_TOO_BIG)
      })
    })

    context('when the app is already initialized', () => {
      before('initialize app', async () => {
        await voting.initialize(token.address, NEEDED_SUPPORT, MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, OVERRULE_WINDOW)
      })

      it('cannot be re-initialized', async () => {
        await assertRevert(voting.initialize(token.address, NEEDED_SUPPORT, MINIMUM_ACCEPTANCE_QUORUM, VOTE_DURATION, OVERRULE_WINDOW), ARAGON_OS_ERRORS.INIT_ALREADY_INITIALIZED)
      })

...
contract('Voting', ([_, owner]) => {
  ...
  describe('roles', () => {
    it('computes roles properly', async () => {
      ...
      assert.equal(await voting.CHANGE_DELEGATED_VOTING_PERIOD_ROLE(), EXPECTED_CHANGE_DELEGATED_VOTING_PERIOD_ROLE, 'CHANGE_DELEGATED_VOTING_PERIOD_ROLE doesn’t match')

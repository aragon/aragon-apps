...
contract('Voting', ([_, owner]) => {
  ...
  describe('roles', () => {
    it('computes roles properly', async () => {
      ...
      const EXPECTED_MODIFY_QUIET_ENDING_ROLE = sha3('MODIFY_QUIET_ENDING_ROLE')
      assert.equal(await voting.MODIFY_QUIET_ENDING_ROLE(), EXPECTED_MODIFY_QUIET_ENDING_ROLE, 'MODIFY_QUIET_ENDING_ROLE doesn’t match')
    })
  })
})

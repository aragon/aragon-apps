...
contract('Finance App', ([root, owner, recipient]) => {
    ...
    context('checking permissions', () => {
        ...
        it('transfer permission', async () => {
            let transferPermission = await acl.hasPermission(finance.address, vault.address, TRANSFER_ROLE);
            assert.equal(transferPermission, true);
        });

        it("should revert when creating a payment from an address that does not have create payments permission", async() => {
          return assertRevert(async () => {
            await finance.newImmediatePayment(token2.address, recipient, amount, '', {from: owner})
            'address does not have permission to create payments'
          })
        });

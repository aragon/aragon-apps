...
const getEventData = (receipt, event, arg) =>
  receipt.logs.filter(log => log.event == event)[0].args[arg];

const bufferedAddress = address =>
  Buffer.from(ethUtil.stripHexPrefix(address), 'hex');
const bufferedInt = int => ethUtil.setLengthLeft(int, 32);
const bufferedString = str => Buffer.from(str, 'utf8');
...
contract('Finance App', accounts => {
  ...
  const newProxyFinance = async () => {
    ...
    const financeApp = Finance.at(
      receipt2.logs.filter(l => l.event == 'NewAppProxy')[0].args.proxy
    );
  ...
  it('records ERC20 deposits', async () => {
    ...
    assert.equal(
      getEventData(receipt, 'NewTransaction', 'reference'),
      'ref',
  ...
  it('records ETH deposits using deposit function', async () => {
    ...
    const transactionId = receipt.logs.filter(
      log => log.event == 'NewTransaction'
    )[0].args.transactionId;
  ...
  it('records ETH deposits using assisted deposit function', async () => {
    const sentWei = 10;
    const reference = 'assisted deposit reference';
    const nonce = 0;
    const privateKey =
      'c87509a1c067bbde78beb793e6fa76530b6382a4c0241e5e4a9ec0a0f44dc0d3';
    const assistedAddressBuffer = ethUtil.privateToAddress(
      Buffer.from(privateKey, 'hex')
    );
    const assistedAddress = ethUtil.addHexPrefix(
      assistedAddressBuffer.toString('hex')
    );
    const parameters = [
      bufferedAddress(ETH),
      bufferedInt(sentWei),
      bufferedString(reference),
      bufferedInt(nonce)
    ];
    const tightlyPackedParams = Buffer.concat(parameters);
    const hashedTightPack = ethUtil.sha3(tightlyPackedParams);
  ...
  context('locked tokens', () => {
    ...
    it('are recovered using Finance#recoverToVault', async () => {
      ...
      assert.equal(
        ...
        initialBalance.plus(5).toString(),
        'deposited tokens must be in vault'
      );
      assert.equal(
        await token1.balanceOf(finance.address),
        0,
    ...
    it('fail to be recovered using AragonApp#transferToVault', async () => {
      return assertRevert(() => finance.transferToVault(token1.address));
    });

    it('fail to be recovered if token balance is 0', async () => {
      // if current balance is zero, it reverts
      return assertRevert(async () => finance.recoverToVault(token2.address));
    });
  });
  ...
  context('locked ETH', () => {
    ...
    beforeEach(async () => {
      ...
      assert.equal(
        (await getBalance(finance.address)).toNumber(),
        lockedETH,
    ...
    it('fails to be recovered using AragonApp#transferToVault', async () => {
      return assertRevert(() => finance.transferToVault(ETH));
    });
    ...
    it('fails to be recovered if ETH balance is 0', async () => {
      ...
      // if current balance is zero, it reverts
      return assertRevert(async () => finance.recoverToVault(ETH));
    });
  });
  ...
  it('before setting budget allows unlimited spending', async () => {
    ...
    assert.equal(
      (await token2.balanceOf(recipient)).toString(),
      amount,
  ...
  it('can change period duration', async () => {
    ...
    assert.equal(
      (await finance.currentPeriodId()).toString(),
      2,
  ...
  it('only transitions as many periods as allowed', async () => {
    ...
    const newPeriodEvents = receipt.logs.filter(
      log => log.event == 'NewPeriod'
    );
    assert.equal(
      newPeriodEvents.length,
      1,
      'should have only emitted one new period event'
    );
    assert.equal(
      await finance.currentPeriodId(),
      1,
  ...
  context('setting budget', () => {
    ...
    it('records payment', async () => {
      ...
      const receipt = await finance.newPayment(
        token1.address,
        recipient,
        amount,
        time,
      ...
      assert.equal(
        getEventData(receipt, 'NewPayment', 'reference'),
        'ref',
        'ref should match'
      );
    ...
    it('can create single payment', async () => {
      ...
      assert.equal(
        (await token1.balanceOf(recipient)).toString(),
        amount,
        'recipient should have received tokens'
      );
    ...
    it('can create recurring payment', async () => {
      ...
      // repeats up to 10 times every 2 seconds
      const firstReceipt = await finance.newPayment(
        token1.address,
        recipient,
        amount,
        time,
      ...
      assert.equal(
        ...
        time + 4 + 2,
        'payment should be repeated again in 2'
      );
      ...
      return Promise.all(
        [firstReceipt, secondReceipt].map(async (receipt, index) => {
          ...
          assert.equal(
            paymentRepeatNumber.toNumber(),
            repeatNum,
            `payment repeat number should be ${repeatNum}`
          );
        })
      );
    });
    ...
    it('can create recurring ether payment', async () => {
      ...
      assert.equal(
        (await getBalance(withdrawAddr)).toString(),
        amount * 3,
    ...
    context('many accounting period transitions', () => {
      ...
      beforeEach(async () => {
        ...
        await finance.mock_setTimestamp(
          time + (maxTransitions + 2) * PERIOD_DURATION
        );
      });
      ...
      it('can transition periods externally to remove deadlock for direct deposits', async () => {
        ...
        assert.equal(
          (await getBalance(vault.address)).toNumber(),
    ...
    const assertPaymentFailure = receipt => {
      const filteredLogs = receipt.logs.filter(
        log => log.event == 'PaymentFailure'
      );
      assert.equal(
        filteredLogs.length,
        1,
        'should have logged payment failure'
      );
    };
  ...
  context('Without initialize', async () => {
    ...
    context('locked tokens', () => {
      ...
      it('fail to be recovered using Finance#recoverToVault', async () => {
        return assertRevert(async () => finance.recoverToVault(token1.address));
      });
    });
    ...
    context('locked ETH', () => {
      ...
      beforeEach(async () => {
        ...
        assert.equal(
          (await getBalance(nonInit.address)).toNumber(),
          lockedETH,
          'finance should have stuck ETH'
        );
      });
      ...
      it('fails to be recovered using Finance#recoverToVault', async () => {
        return assertRevert(async () => await nonInit.recoverToVault(ETH));
      });

...
contract('Survey app', ([root, holder1, holder2, holder19, holder31, holder50, nonHolder]) => {
  ...
  beforeEach(async () => {
    ...
    await acl.revokePermission(ANY_ENTITY, survey.address, CREATE_SURVEYS_ROLE, {from: root});
    await acl.revokePermission(ANY_ENTITY, survey.address, MODIFY_PARTICIPATION_ROLE, {from: root});
  ...
  afterEach(async () => {
    await acl.grantPermission(ANY_ENTITY, survey.address, CREATE_SURVEYS_ROLE, {from: root});
    await acl.grantPermission(ANY_ENTITY, survey.address, MODIFY_PARTICIPATION_ROLE, {from: root});
  })

  context('check ACL permissions', async() => {

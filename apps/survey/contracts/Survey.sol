...
contract Survey is AragonApp {
    ...
    bytes32 public constant CREATE_SURVEYS_ROLE =
        keccak256("CREATE_SURVEYS_ROLE");
    bytes32 public constant MODIFY_PARTICIPATION_ROLE =
        keccak256("MODIFY_PARTICIPATION_ROLE");

    uint64 public constant PCT_BASE = 10**18; // 0% = 0; 1% = 10^16; 100% = 10^18
    uint256 public constant ABSTAIN_VOTE = 0;

    string private constant ERROR_MIN_PARTICIPATION =
        "SURVEY_MIN_PARTICIPATION";
    ...
    string private constant ERROR_VOTE_WRONG_OPTION =
        "SURVEY_VOTE_WRONG_OPTION";
    string private constant ERROR_NO_STAKE = "SURVEY_NO_STAKE";
    string private constant ERROR_OPTIONS_NOT_ORDERED =
        "SURVEY_OPTIONS_NOT_ORDERED";
    ...
    modifier acceptableMinParticipationPct(uint64 _minParticipationPct) {
        require(
            _minParticipationPct > 0 && _minParticipationPct <= PCT_BASE,
            ERROR_MIN_PARTICIPATION
        );
        _;
    }
    ...
    function initialize(
        ...
        uint64 _surveyTime,
        uint256 epoch
    ) external onlyInit acceptableMinParticipationPct(_minParticipationPct) {
        initialized(epoch);
    ...
    function changeMinAcceptParticipationPct(uint64 _minParticipationPct)
        ...
        authP(
            ...
            arr(uint256(_minParticipationPct), uint256(minParticipationPct))
        )
     ...
     */
    function newSurvey(string _metadata, uint256 _options)
        external
        auth(CREATE_SURVEYS_ROLE)
    ...
    function voteOptions(
        ...
        uint256[] _optionIds,
        uint256[] _stakes
    ) external surveyExists(_surveyId) {
        require(
            _optionIds.length == _stakes.length && _optionIds.length > 0,
            ERROR_VOTE_WRONG_INPUT
        );
        require(canVote(_surveyId, msg.sender), ERROR_CAN_NOT_VOTE);
     ...
     */
    function voteOption(uint256 _surveyId, uint256 _optionId)
        external
        surveyExists(_surveyId)
    {
        ...
        uint256 voterStake =
            token.balanceOfAt(msg.sender, survey.snapshotBlock);
    ...
    function canVote(uint256 _surveyId, address _voter)
        public
        view
        surveyExists(_surveyId)
        returns (bool)
    {
        ...
        return
            _isSurveyOpen(survey) &&
            token.balanceOfAt(_voter, survey.snapshotBlock) > 0;
    }
    ...
    function getOptionPower(uint256 _surveyId, uint256 _optionId)
        public
        view
        surveyExists(_surveyId)
        returns (uint256)
    ...
    function isParticipationAchieved(uint256 _surveyId)
        public
        view
        surveyExists(_surveyId)
        returns (bool)
    {
        ...
        uint256 participationPct =
            survey.participation.mul(PCT_BASE) / survey.votingPower;
    ...
    function _resetVote(uint256 _surveyId) internal {
        ...
        if (previousVote.optionsCastedLength > 0) {
            ...
            for (uint256 i = 1; i <= previousVote.optionsCastedLength; i++) {
                OptionCast storage previousOptionCast =
                    previousVote.castedVotes[i];
                uint256 previousOptionPower =
                    survey.optionPower[previousOptionCast.optionId];
                uint256 currentOptionPower =
                    previousOptionPower.sub(previousOptionCast.stake);
                survey.optionPower[
                    previousOptionCast.optionId
                ] = currentOptionPower;
            ...
            uint256 voterStake =
                token.balanceOfAt(msg.sender, survey.snapshotBlock);
            uint256 previousParticipation =
                voterStake.sub(previousVote.castedVotes[0].stake);
            // And remove it from total participation
            survey.participation = survey.participation.sub(
    ...
    function _voteOptions(
        ...
        senderVotes.castedVotes[0] = OptionCast({
            optionId: ABSTAIN_VOTE,
            stake: 0
        });
        for (
            uint256 optionIndex = 1;
            optionIndex <= _optionIds.length;
            optionIndex++
        ) {
            ...
            require(
                optionId != ABSTAIN_VOTE && optionId <= survey.options,
            ...
            // we added
            require(
                senderVotes.castedVotes[optionIndex - 1].optionId < optionId,
                ERROR_OPTIONS_NOT_ORDERED
            );

            // Register voter amount
            senderVotes.castedVotes[optionIndex] = OptionCast({
                optionId: optionId,
                stake: stake
            });

            // Add to total option support
            survey.optionPower[optionId] = survey.optionPower[optionId].add(
                stake
            );
    ...
    function _isSurveyOpen(SurveyStruct storage _survey)
        internal
        view
        returns (bool)
    {
